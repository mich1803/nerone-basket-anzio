  ...
  return <main>
    ...
    <section className="game-hero">
      ...
      <div className="scoreboard">
        <div><img src={assetPath(homeLogo)} alt="" /><h1>{home?.short_name ?? game.home_team}</h1></div>
        <strong>{game.home_score ?? '–'}<span>:</span>{game.away_score ?? '–'}</strong>
        <div><img src={assetPath(awayLogo)} alt="" /><h1>{away?.short_name ?? game.away_team}</h1></div>
      </div>
      <div className="game-meta">
        <span><CalendarDays size={17} /> {game.date ? `${formatGameDate(game.date)}${game.time ? ` · ${game.time}` : ''}` : 'Data non disponibile'}</span>
        <span><MapPin size={17} /> {game.venue || game.address || 'Luogo non disponibile'}</span>
